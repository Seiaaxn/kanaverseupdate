...
const StreamingDonghuaRelatedEpisodes = ({ episodes = [], currentEpisodeNumber, currentEpisodeUrl, onEpisodeClick }) => {
    const [viewMode, setViewMode] = useState('scroll');

    if (!episodes || episodes.length === 0) return null;

    // Ekstrak nomor dari berbagai format field & URL
    const getNum = (ep, idx) => {
    ...
    // Cek apakah episode ini yang sedang aktif
    const isCurrent = (ep, idx) => {
        if (currentEpisodeUrl && ep.url) return ep.url === currentEpisodeUrl;
        return String(getNum(ep, idx)) === String(currentEpisodeNumber);
    };

    // Validasi URL episode masih di domain yang sama
    const isSameDomain = (url) => {
        if (!currentEpisodeUrl || !url) return true;
        try {
            return new URL(currentEpisodeUrl).hostname === new URL(url).hostname;
        } catch { return true; }
    };
            ...
            {viewMode === 'scroll' && (
                <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
                    {episodes.map((ep, idx) => {
                        ...
                        const date = formatDate(ep.date || ep.releaseDate || ep.aired);
                        const active = isCurrent(ep, idx);
                        const valid = isSameDomain(ep.url);
                        return (
                            <button
                                key={idx}
                                onClick={() => valid && onEpisodeClick(ep)}
                                ...
                                <p className="text-sm font-bold leading-tight"
                                    style={{ color: active ? '#a899fc' : 'white' }}>
                                    Episode {num}
                                </p>
                                {date && (
